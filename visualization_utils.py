...
def visualize_boxes_and_labels_on_image_array(
  ...
  #boxes = np_box_list.BoxList(boxes)  
  #boxes.add_field('scores', scores)
  #boxes = np_box_list_ops.non_max_suppression(
        #boxes, 1000000, 1.0)
  
  if not max_boxes_to_draw:
    max_boxes_to_draw = boxes.shape[0]  #len(boxes) 
  if not isinstance(boxes,np.ndarray):
    length_ = boxes.num_boxes()
    boxes = boxes.get()
  else:
    length_ = boxes.shape[0]
  
  for i in range(length_):  #range(boxes.num_boxes()): 
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        if agnostic_mode:
          ...
          if classes[i]==5:
            box_to_color_map[box] = STANDARD_COLORS[50] #classes[i] % len(STANDARD_COLORS)]
          elif classes[i]==4:
            box_to_color_map[box] = STANDARD_COLORS[-24]
          elif classes[i]==3:
            box_to_color_map[box] = STANDARD_COLORS[-26]
          elif classes[i]==2:
            box_to_color_map[box] = STANDARD_COLORS[-2]
          elif classes[i]==1:
            box_to_color_map[box] = STANDARD_COLORS[16] #-8reddont,-6vio,16red,-12,5,13choc,-25oradont,-no1415,-1green,-32pink
          elif classes[i]==6:
            box_to_color_map[box] = STANDARD_COLORS[-4]
          elif classes[i]==7:
            box_to_color_map[box] = STANDARD_COLORS[12]
  ...
  for box, color in box_to_color_map.items():
    ...
    draw_bounding_box_on_image_array(
        ...
        use_normalized_coordinates=use_normalized_coordinates)
    print('drawinggggggggggggggggggggg')
    #draw_bounding_box_on_image_array(
        #image,
        #0,
        #458.75,
