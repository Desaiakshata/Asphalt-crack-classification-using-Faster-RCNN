...
from object_detection.utils import crack_category
...
def visualize_detection_results(result_dict,
  ...
  #reading pavement markings for the image
  filename = str(result_dict[input_fields.key])
  print(filename[2:-1])
  book = xlrd.open_workbook('/content/markings.xls')
  sheet = book.sheet_by_name('Sheet 1')
  k=sheet.col_values(0)
  ind = k.index(filename[2:-1])
  l = sheet.row_values(ind)
  marking_list= l[1:7]
  ...
  print('-------------------------------------------------------------------')
  detection_boxes1 = np_box_list.BoxList(detection_boxes)
  detection_boxes1.add_field('scores', detection_scores)
  detection_boxes1.add_field('classes', detection_classes)
  detection_boxes1 = np_box_list_ops.non_max_suppression(
        detection_boxes1, 100000, 0.5)
  detection_boxes_1 = detection_boxes1.get_field('boxes')
  detection_classes1 = detection_boxes1.get_field('classes')
  detection_scores1 = detection_boxes1.get_field('scores')
  print(detection_classes1)
  detection_boxes_arr,detection_classes1,detection_scores1 = crack_category.remove_overlaps(detection_boxes_1,detection_classes1,detection_scores1,marking_list)
  print(detection_classes1)
  print(detection_boxes_arr.shape[0])
  box_list_left_load,extent_left,box_list_right_load,extent_right,block,extent_block,b1,b1_extent = crack_category.cracks(detection_boxes_arr,detection_classes1,detection_scores1,marking_list)
  
  detection_boxes2 = np_box_list.BoxList(detection_boxes_arr)
  ...
  tf.logging.info('Detection visualizations written to summary with tag %s.',
                  tag)
  return box_list_left_load,extent_left,box_list_right_load,extent_right,block,extent_block,b1,b1_extent
